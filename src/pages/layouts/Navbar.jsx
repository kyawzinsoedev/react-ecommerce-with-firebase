import search from "../../assets/search.svg";

const Navbar = () => {
  return (
    <nav className="sticky top-0 z-50 bg-white/80 backdrop-blur-md border-b border-slate-100">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between h-20 items-center">
          {/* Logo - Using Primary Dark & Medium */}
          <div className="text-2xl font-bold text-primary-dark tracking-tighter">
            Source<span className="text-primary-medium">Hub</span>
          </div>

          {/* Navigation Links */}
          <div className="hidden md:flex space-x-10 font-medium text-slate-600">
            <a
              href="#"
              className="hover:text-primary-deep transition-colors relative group"
            >
              Home
              <span className="absolute -bottom-1 left-0 w-0 h-0.5 bg-primary-medium transition-all group-hover:w-full"></span>
            </a>
            <a
              href="#"
              className="hover:text-primary-deep transition-colors relative group"
            >
              Shop
              <span className="absolute -bottom-1 left-0 w-0 h-0.5 bg-primary-medium transition-all group-hover:w-full"></span>
            </a>
            <a
              href="#"
              className="hover:text-primary-deep transition-colors relative group"
            >
              Categories
              <span className="absolute -bottom-1 left-0 w-0 h-0.5 bg-primary-medium transition-all group-hover:w-full"></span>
            </a>
          </div>

          {/* Right Side Actions */}
          <div className="flex items-center space-x-6">
            {/* Search Icon */}
            <button className="p-2.5 text-slate-500 hover:bg-slate-50 rounded-full transition-all">
              <img src={search} alt="Search" className="w-8 h-8 " />
            </button>

            {/* Profile Image - Rounded & Styled */}
            <button className="flex items-center border-2 border-transparent hover:border-primary-light rounded-full transition-all overflow-hidden">
              <img
                src="data:image/png;base64,"
                alt="Profile"
                className="w-10 h-10 object-cover"
              />
            </button>

            {/* Cart Button - Using Palette Primary Dark */}
            <div className="relative">
              <button className="bg-primary-deep text-white px-5 py-2.5 rounded-xl text-sm font-bold hover:bg-primary-deep transition-all shadow-lg shadow-primary-dark/20 flex items-center gap-2">
                <span>Cart</span>
                <span className="bg-primary-light text-primary-dark px-1.5 py-0.5 rounded-md text-[10px]">
                  0
                </span>
              </button>
            </div>
          </div>
        </div>
      </div>
    </nav>
  );
};

export default Navbar;
